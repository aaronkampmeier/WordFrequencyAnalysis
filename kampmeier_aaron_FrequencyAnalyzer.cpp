...
FrequencyAnalyzer::FrequencyAnalysisResults::FrequencyAnalysisResults(const char *inputFile, int fileWordCount,
		...
		: _wordFrequenciesTree(wordFrequencies), _inputFile(copyString(inputFile)), _fileWordCount(fileWordCount),
		  analyzedOn(time(nullptr)) {
	_wordFrequenciesTreeOrdered = nullptr;
}

/**
 * Frees the _inputFile path name and the frequency tree
 */
...
const char *FrequencyAnalyzer::FrequencyAnalysisResults::getInputFilePath() {
	return _inputFile;
}
...
FrequencyAnalyzer::FrequencyAnalysisResults::wordsOrderedByFrequency(int &returnLength) {
	...
	if (frequencyOrderedTree == nullptr) {
		...
		auto getUniqueFrequencyKey = [](const WordFrequency &val) -> BinarySearchTree<WordFrequency>::StorageKey {
			...
			int maskStart = sizeof(StorageKey) - sizeof(int);
			...
			StorageKey hashMask = ~(((1ul << (sizeof(int) * 8)) - 1) << ((maskStart * 8)));
			StorageKey maskedHash = val.hash & hashMask;
			
			// Shift frequency bits into place
			StorageKey shiftedFreq = ((unsigned long) val.frequency) << (maskStart * 8);
...
std::ostream &operator<<(std::ostream &os, FrequencyAnalyzer::FrequencyAnalysisResults &results) {
	os << "\nAnalysis for file: " << results._inputFile << std::endl;
...
bool FrequencyAnalyzer::FrequencyAnalysisResults::exportReportTo(const char *outputFilePath) {
	...
	fprintf(outputFile, "\n\nAnalysis for file: %s", _inputFile);
...
FrequencyAnalyzer::FrequencyAnalysisResults::WordFrequency::WordFrequency(const char *word): word
		(FrequencyAnalyzer::FrequencyAnalysisResults::copyString(word)), hash(_hash(
